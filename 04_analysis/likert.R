...
plotLikert <- function(dimension, aspect, features, featuresC, likertC, featuresN, likertN, featureCurrent){
  ...
  fileName = paste("../05_output/likert", paste(paste(str_pad(index, 2, pad="0"), dimension, aspect, sep="__"), "pdf", sep="."), sep="/")
...
needTxtLatex <- function(dimension, aspect, features, featuresC, likertC, featuresN, likertN, featureCurrent, featureNeeds){
  ...
  needFileName = paste("../05_output/tables-txt", paste(paste(str_pad(index, 2, pad="0"), dimension, aspect, sep="__"), "txt", sep="."), sep="/")
  write.table(diff, file = needFileName, sep = "\t", row.names = FALSE)
  
  needLatexFileName = paste("../05_output/tables-latex", paste(paste(str_pad(index, 2, pad="0"), dimension, aspect, sep="__"), "tex", sep="."), sep="/")
...
write.xlsx(allDiffsToPrint, "../05_output/aggregated/xlsx/Aggregated.xlsx", sheetName = "Sheet1", col.names = TRUE, row.names = FALSE, append = TRUE)
latexTable(diffData = allDiffsToPrint[order(-allDiffsToPrint$current),], fileName="../05_output/tables-latex/aggregated/aggregated-currentDesc.tex", caption="Most frequently encountered features across the three dimensions", label="most-frequent", dimension = TRUE)
latexTable(diffData = allDiffsToPrint[order(allDiffsToPrint$current),], fileName="../05_output/tables-latex/aggregated/aggregated-currentAsc.tex", caption="Least frequently encountered features across the three dimensions", label="least-frequent", dimension = TRUE)
latexTable(diffData = allDiffsToPrint[order(-allDiffsToPrint$need),], fileName="../05_output/tables-latex/aggregated/aggregated-needDesc.tex", caption="Most needed features across the three dimensions", label="most-needed", dimension = TRUE)
latexTable(diffData = allDiffsToPrint[order(allDiffsToPrint$need),], fileName="../05_output/tables-latex/aggregated/aggregated-needAsc.tex", caption="Least needed features across the three dimensions", label="least-needed", dimension = TRUE)
latexTable(diffData = allDiffsToPrint[order(-allDiffsToPrint$diff),], fileName="../05_output/tables-latex/aggregated/aggregated-diffDesc.tex", caption="Most impactful shortcomings across the three dimensions", label="most-impactful", dimension = TRUE)
latexTable(diffData = allDiffsToPrint[order(allDiffsToPrint$diff),], fileName="../05_output/tables-latex/aggregated/aggregated-diffAsc.tex", caption="Least impactful shortcomings across the three dimensions", label="least-impactful", dimension = TRUE)


d = allDiffsToPrint[order(-allDiffsToPrint$current), ]
latexTopBottomTable(diffData = d[, c(1:3,4)], "Current", fileName="../05_output/tables-latex/aggregated/top-bottom-current.tex", caption="The ten most used, and the ten least used features across the three dimensions", label="top-bottom-current", dimension = TRUE)
...
for(i in 1:length(dimensions)){
  ...
  outputFile <- paste(paste("../05_output/plots/scatterplot_", tolower(dimensions[[i]]), sep=""), ".pdf", sep="")
